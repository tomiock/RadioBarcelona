"""
Train a LineCNN to classify text lines as handwritten (H) or typewritten (T).

Same architecture as train_line_classifier.py; only the label map differs.
Labels come from label_handwritten_type.py (ht_labels.json).

Usage:
    conda activate laypa
    python scripts/train_ht_classifier.py \
        ...
        --output   /data/storage/users/tockier/laypa_ht_classify/model \
        --epochs   30 \
        --batch    16

Best checkpoint (by val accuracy) saved as <output>/best.pth.
"""
...
def load_entries(manifest_path: str, labels_path: str) -> list[dict]:
    ...
    label_map = {"H": 0, "T": 1}   # H=handwritten, T=typewritten
    entries = []
...
def main():
    ...
    t_train = sum(1 for e in trn_set if e["label"] == 1)
    print(f"Train: {len(trn_set)} (H={h_train}, T={t_train})  |  Val: {len(val_set)}")

    class_counts = [h_train, t_train]
