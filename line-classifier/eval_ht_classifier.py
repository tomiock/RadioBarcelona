"""
Run the trained H/T LineCNN on all labeled crops and produce a self-contained
HTML report for manual inspection.

Shows every crop with its ground-truth label (H/T), predicted label, and
confidence. Misclassifications are highlighted. Useful for inspecting errors
on 1936–1939 mixed-era documents where the boundary between H and T is hardest.

Usage:
    conda activate laypa
    python scripts/eval_ht_classifier.py \
        --manifest   /data/storage/users/tockier/laypa_ht_classify/ht_manifest.json \
        --labels     /data/storage/users/tockier/laypa_ht_classify/ht_labels.json \
        --checkpoint /data/storage/users/tockier/laypa_ht_classify/model/best.pth \
        --output     /data/storage/users/tockier/laypa_ht_classify/eval_report.html
"""
...
# ── model (must match train_ht_classifier.py) ─────────────────────────────────
...
<head>
<meta charset="UTF-8">
<title>H/T Classifier — Evaluation Report</title>
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .pred-T {{ color: #64b5f6; font-weight: bold; }}
  .gt-H {{ color: #a5d6a7; }}
  .gt-T {{ color: #90caf9; }}
...
<body>
<h1>H/T Line Classifier — Evaluation Report</h1>
...
def main():
    ...
    if not entries:
        print("No labeled entries found. Check --manifest and --labels paths.")
        sys.exit(1)
    ...
    h_correct = sum(1 for r in results if r["gt"] == "H" and r["correct"])
    h_total   = sum(1 for r in results if r["gt"] == "H")
    t_correct = sum(1 for r in results if r["gt"] == "T" and r["correct"])
    t_total   = sum(1 for r in results if r["gt"] == "T")
    print(f"  H: {h_correct}/{h_total} = {h_correct/h_total:.1%}" if h_total else "  H: no samples")
    print(f"  T: {t_correct}/{t_total} = {t_correct/t_total:.1%}" if t_total else "  T: no samples")
