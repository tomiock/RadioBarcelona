"""
Crop text lines from Laypa PageXML output and serve a browser UI for binary
classification: H = handwritten, T = typewritten.

Sample across multiple era folders (e.g. 1931, 1936, 1938, 1944, 1950) to get
balanced coverage of both classes. Merge manifests from multiple runs before
training if needed.

Usage:
    conda activate laypa
    python scripts/label_handwritten_type.py \
        ...
        --output   /data/storage/users/tockier/laypa_ht_classify \
        --n        200 \
        --port     5054

Then open http://localhost:5054 in your browser.
Keys:  H → handwritten   T → typewritten   ← Backspace → undo last
Results saved to <output>/ht_labels.json after every classification.
"""
...
def get_arguments() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="H/T line classification UI")
    ...
    p.add_argument("--output",   required=True, help="Output dir (crops + ht_labels.json)")
    p.add_argument("--n",        type=int, default=200, help="Number of lines to sample")
    p.add_argument("--port",     type=int, default=5054, help="Flask port")
...
def prepare_crops(args, output_dir: Path) -> list[dict]:
    ...
    manifest_path = output_dir / "ht_manifest.json"
...
<head>
<meta charset="UTF-8">
<title>H/T Line Classifier</title>
<style>
...
function updateUI() {
  ...
  else if (lbl === 'T') { badge.textContent = 'TYPEWRITTEN'; badge.className = 'badge-t'; }
...
function showDone() {
  ...
  document.getElementById('done-msg').textContent =
    labeled + ' lines classified. Results saved to ht_labels.json.';
}

document.addEventListener('keydown', e => {
  if (e.key === 'h' || e.key === 'H') classify('H');
  else if (e.key === 't' || e.key === 'T') classify('T');
...
def build_app(manifest: list[dict], output_dir: Path):
    ...
    app = Flask(__name__)
    labels_path = output_dir / "ht_labels.json"
...
def main():
    ...
    print("Keys: H = handwritten, T = typewritten, Backspace = undo\n")
